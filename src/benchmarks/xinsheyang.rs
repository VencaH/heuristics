...
// source:  https://benchmarkfcns.info/doc/xinsheyangn1fcn.html
//
#[derive(Debug)]
pub struct XinSheYang {
    min: f32,
    max: f32,
    dim: usize,
...
impl HasBuilder<XinSheYang> for XinSheYang {}

impl Benchmark for XinSheYang {
    const FUNCTION_NAME: &'static str = "Xin-She Yang function";
    fn get_min(&self) -> f32 {
        self.min
    }
    ...
    fn cost_function(&self, input: &[f32]) -> f32 {
        input.iter().enumerate().map(|(i,&x)| random::<f32>().mul(
                x.abs()
                .powi(i as i32)
                )
            )
...
mod test {
    ...
    #[test]
    fn random() {
        let xin_she_yang = XinSheYang::builder()
            .minimum(-500f32)
            .maximum(500f32)
            .dimensions(5usize)
            .build()
            .unwrap();
        let random_1 = xin_she_yang.get_random();
        let random_2 = xin_she_yang.get_random();
        let random_3 = xin_she_yang.get_random();
    ...
    fn local_next() {
        let xin_she_yang = XinSheYang::builder()
            ...
            .unwrap();
        let random_1 = xin_she_yang.get_random();
        let new_local = xin_she_yang.get_local_next(&random_1);
    ...
    fn local_next_in_range() {
        ...
        let random_1 = xin_she_yang.get_random();
        let new_local = xin_she_yang.get_local_next(&random_1);
        let new_low = xin_she_yang.get_local_next(&vec![-500.0, -500.0, -500.0, -500.0, -500.0]);
        let new_high = xin_she_yang.get_local_next(&vec![500.0, 500.0, 500.0, 500.0, 500.0]);
